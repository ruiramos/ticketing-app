...
async function setupMocks(page: Page, eventData: MockEvent | null) {
  await page.route('**/api/trpc/event.byId,user.getUserEvent*', async (route) => {
    ...
    if (url.includes('event.byId')) {
      if (eventData) {
        ...
        jsonResponsePayload = [{ error: { json: { message: 'Event not found', code: 'NOT_FOUND' } } }];
      }
      ...
      const userEventMockData = {
        ...
        variants: eventData?.variants.map(v => ({
          id: v.id,
  ...
  await page.route('**/api/trpc/event.update*', async (route) => {
    ...
    await route.fulfill({
      contentType: 'application/json',
      body: JSON.stringify([{ result: { data: { id: MOCK_EVENT_ID, ...route.request().postDataJSON()?.['0']?.json } } }]),
    });
  });
}
...
async function getVariantTitles(page: Page): Promise<string[]> {
  ...
  const variantTitleInputs = await page.locator('input[id^="variant-title-"]').elementHandles();
  const titles = [];
...
test.describe('Event Edit Page - Variant Reordering', () => {
  ...
  test.describe('Initial Sort Order', () => {
    test('1.1: should render variants sorted by displayOrder', async ({ page }) => {
      const variants: MockVariant[] = [
        { id: 'v2', title: 'Variant B (Order 0)', price: 10, stock: 100, displayOrder: 0 },
        { id: 'v3', title: 'Variant C (Order 1)', price: 20, stock: 100, displayOrder: 1 },
        { id: 'v1', title: 'Variant A (Order 2)', price: 30, stock: 100, displayOrder: 2 },
      ];
      // The component should re-sort these based on displayOrder on load
      const expectedOrder = ['Variant B (Order 0)', 'Variant C (Order 1)', 'Variant A (Order 2)'];
    ...
    test('1.2: should handle variants with null/undefined displayOrder (sorting them by originalIndex at the end)', async ({ page }) => {
      const variants: MockVariant[] = [
        { id: 'v1', title: 'Variant A (No displayOrder)', price: 10, stock: 100, displayOrder: null }, // Should be originalIndex 0
        { id: 'v2', title: 'Variant B (displayOrder 0)', price: 20, stock: 100, displayOrder: 0 },
        { id: 'v3', title: 'Variant C (No displayOrder)', price: 30, stock: 100 }, // Should be originalIndex 2
        { id: 'v4', title: 'Variant D (displayOrder 1)', price: 40, stock: 100, displayOrder: 1 },
      ];
  ...
  test.describe('Reordering with Up/Down Buttons', () => {
    ...
    test('2.1: should move a variant up and update order', async ({ page }) => {
      ...
      await expect(await getVariantTitles(page)).toEqual(['Variant 1', 'Variant 2', 'Variant 3']);
      ...
      await page.locator('.flex.items-end.gap-2.p-4.border.rounded').nth(1).locator('button[title="Move variant up"]').click();
      
      expect(await getVariantTitles(page)).toEqual(['Variant 2', 'Variant 1', 'Variant 3']);
      
      // Check button states: "Variant 2" (now first) should have "Up" disabled
      const firstVariantItem = page.locator('.flex.items-end.gap-2.p-4.border.rounded').nth(0);
      await expect(firstVariantItem.locator('button[title="Move variant up"]')).toBeDisabled();
    });

    test('2.2: should move a variant down and update order', async ({ page }) => {
      ...
      await page.locator('.flex.items-end.gap-2.p-4.border.rounded').nth(1).locator('button[title="Move variant down"]').click();
      
      expect(await getVariantTitles(page)).toEqual(['Variant 1', 'Variant 3', 'Variant 2']);

      // Check button states: "Variant 2" (now last) should have "Down" disabled
      const lastVariantItem = page.locator('.flex.items-end.gap-2.p-4.border.rounded').nth(2);
      await expect(lastVariantItem.locator('button[title="Move variant down"]')).toBeDisabled();
    });

    test('2.3: Up/Down buttons should be disabled for first/last variant respectively', async ({ page }) => {
      await setupMocks(page, createMockEventData([...initialVariants]));
      await page.goto(EVENT_EDIT_URL);
      ...
      const firstVariantItem = page.locator('.flex.items-end.gap-2.p-4.border.rounded').nth(0);
      await expect(firstVariantItem.locator('button[title="Move variant up"]')).toBeDisabled();
      await expect(firstVariantItem.locator('button[title="Move variant down"]')).toBeEnabled();

      // Middle variant ("Variant 2")
      const middleVariantItem = page.locator('.flex.items-end.gap-2.p-4.border.rounded').nth(1);
      await expect(middleVariantItem.locator('button[title="Move variant up"]')).toBeEnabled();
      await expect(middleVariantItem.locator('button[title="Move variant down"]')).toBeEnabled();
      
      // Last variant ("Variant 3")
      const lastVariantItem = page.locator('.flex.items-end.gap-2.p-4.border.rounded').nth(2);
      await expect(lastVariantItem.locator('button[title="Move variant up"]')).toBeEnabled();
      await expect(lastVariantItem.locator('button[title="Move variant down"]')).toBeDisabled();
    });
  });

  test.describe('Display Order after Adding a Variant', () => {
    test('3.1: should add new variant to the end with correct displayOrder (inferred by position)', async ({ page }) => {
      const initial: MockVariant[] = [
        { id: 'v1', title: 'First Variant', price: 10, stock: 10, displayOrder: 0 },
      ];
      await setupMocks(page, createMockEventData(initial));
      await page.goto(EVENT_EDIT_URL);

      await expect(await getVariantTitles(page)).toEqual(['First Variant']);
      ...
      const newVariantItem = page.locator('.flex.items-end.gap-2.p-4.border.rounded').nth(1);
      await expect(newVariantItem.locator('button[title="Move variant down"]')).toBeDisabled();
      // And "Up" button enabled (since there are 2 variants)
      await expect(newVariantItem.locator('button[title="Move variant up"]')).toBeEnabled();
    });
  });

  test.describe('Display Order after Removing a Variant', () => {
    test('4.1: should update displayOrder of remaining variants (inferred by position)', async ({ page }) => {
      const initial: MockVariant[] = [
        { id: 'v1', title: 'Variant X', price: 10, stock: 10, displayOrder: 0 },
        { id: 'v2', title: 'Variant Y (to remove)', price: 20, stock: 20, displayOrder: 1 },
        { id: 'v3', title: 'Variant Z', price: 30, stock: 30, displayOrder: 2 },
      ];
      await setupMocks(page, createMockEventData(initial));
      await page.goto(EVENT_EDIT_URL);

      await expect(await getVariantTitles(page)).toEqual(['Variant X', 'Variant Y (to remove)', 'Variant Z']);
      ...
      await page.locator('.flex.items-end.gap-2.p-4.border.rounded').nth(1).locator('button[title="Delete variant"]').click();
      ...
      const lastVariantItem = page.locator('.flex.items-end.gap-2.p-4.border.rounded').nth(1);
      await expect(lastVariantItem.locator('button[title="Move variant down"]')).toBeDisabled();
      // And "Up" button should be enabled as it's not the first item (total 2 items)
      await expect(lastVariantItem.locator('button[title="Move variant up"]')).toBeEnabled(); 
    });

    test('4.2: should prevent removing the last variant and show alert / disable button', async ({ page }) => {
      const initial: MockVariant[] = [
        { id: 'v1', title: 'The Only Variant', price: 10, stock: 10, displayOrder: 0 },
      ];
      await setupMocks(page, createMockEventData(initial));
      await page.goto(EVENT_EDIT_URL);

      await expect(await getVariantTitles(page)).toEqual(['The Only Variant']);
      
      const variantItem = page.locator('.flex.items-end.gap-2.p-4.border.rounded').nth(0);
      const deleteButton = variantItem.locator('button[aria-label*="Delete variant"], button[title*="Delete variant"], button[title*="delete last variant"]');

      // Check if the button is disabled and has the correct title
      await expect(deleteButton).toBeDisabled();
      await expect(deleteButton).toHaveAttribute('title', 'Cannot delete last variant');
