...
const LAST_FEW_STOCK_WARNING = 5; // Assuming this value from app constants

interface MockOrder {
  id: string;
  status: 'CREATED' | 'COMPLETED' | 'SAVED' | 'APPROVED' | 'PAYER_ACTION_REQUIRED' | 'VOIDED';
  purchaseUnits?: { items?: { name: string; quantity: string }[]; amount?: { value: string } }[];
}
...
const createMockEvent = (config: Partial<MockEventData> = {}): MockEventData => ({
  id: MOCK_EVENT_ID,
  ...
  endsAt: new Date(Date.now() + 7 * 24 * 60 * 60 * 1000 + 8 * 60 * 60 * 1000).toISOString(), // Duration 8 hours
  enabled: true,
  variants: [
    { id: 'v1', title: 'Early Bird Pass', price: 199, stock: 100, displayOrder: 0 },
    { id: 'v2', title: 'Standard Pass', price: 299, stock: 250, displayOrder: 1 },
    { id: 'v3', title: 'VIP Pass', price: 499, stock: 50, displayOrder: 2 },
  ],
  eventExtras: [
    { id: 'e1', title: 'Official Event T-Shirt', price: 25, description: 'High-quality cotton t-shirt with event logo. Available in S, M, L, XL.', currency: 'GBP' },
    { id: 'e2', title: 'Sticker Pack', price: 5, description: null, currency: 'GBP' },
    { id: 'e3', title: 'Catered Lunch Voucher', price: 18, description: 'Enjoy a delicious catered lunch on site.', currency: 'GBP'},
  ],
  ...config,
});

async function setupMocks(page: Page, eventData: MockEventData, orderMocks?: { create?: any, capture?: any, getOrder?: any }) {
  // More robust matcher for event.byId
  await page.route(
    (url) => url.href.includes('/api/trpc/event.byId') && url.searchParams.get('input')?.includes(`"id":"${MOCK_EVENT_ID}"`),
    async (route) => {
      await route.fulfill({ status: 200, json: [{ result: { data: { json: eventData } } }] });
    }
  );
  ...
  await page.route('**/api/trpc/order.createOrder*', async route => {
    if (orderMocks?.create?.error) {
      await route.fulfill({
        status: 200,
        json: [{ error: { json: { message: orderMocks.create.error, ...orderMocks.create.details } } }],
      });
    } else {
      await route.fulfill({ json: [{ result: { data: { json: { id: 'mock_order_id_123', status: 'CREATED', ...orderMocks?.create?.success } } } }] });
    }
  });
  ...
  await page.route('**/api/trpc/order.captureOrder*', async route => {
     if (orderMocks?.capture?.error) {
      await route.fulfill({
        status: 200,
        json: [{ error: { json: { message: orderMocks.capture.error, ...orderMocks.capture.details } } }],
      });
    } else {
      const mockOrderResult: MockOrder = {
        id: 'mock_order_id_123',
        status: 'COMPLETED',
        purchaseUnits: [{ items: [{name: 'Test Item', quantity: '1'}] , amount: {value: '10.00'}}],
        ...orderMocks?.capture?.success
      };
      await route.fulfill({ json: [{ result: { data: { json: mockOrderResult } } }] });
    }
  });
   ...
   await page.route('**/api/trpc/order.byId*', async route => {
    ...
    const orderDataToReturn = orderMocks?.getOrder?.success ?? orderMocks?.capture?.success ?? defaultOrderData;
    await route.fulfill({ json: [{ result: { data: { json: orderDataToReturn } } }] });
  });
}


test.describe('Event Detail Page UI/UX', () => {
  ...
  test.describe('EventItem Layout', () => {
    ...
    test('1.1: should have two-column layout on desktop', async ({ page }) => {
      ...
      await expect(leftColumn.getByRole('heading', { name: eventData.title })).toBeVisible();
      await expect(leftColumn.getByText(eventData.text.substring(0,50))).toBeVisible(); // Check part of text
    ...
    test('1.2: should have single-column layout on mobile', async ({ page }) => {
      await page.setViewportSize({ width: 375, height: 667 }); // Mobile viewport
      await page.goto(EVENT_PAGE_URL);
      
      const eventItemContainer = page.locator('div > .flex.flex-col.md\\:flex-row').first(); // More specific if nested
      await expect(eventItemContainer).toBeVisible();
      ...
      if (leftBox && rightBox) {
        ...
        expect(rightBox.y).toBeGreaterThanOrEqual(leftBox.y + leftBox.height - 5); // -5 for tolerance
      }
    });
  });

  test.describe('EventForm - Dynamic Variant Display', () => {
    test('2.1.1: should render radio buttons for <= 3 variants', async ({ page }) => {
      const eventData = createMockEvent({
        variants: [
          { id: 'v1', title: 'Variant A', price: 10, stock: 10, displayOrder: 0 },
          { id: 'v2', title: 'Variant B', price: 20, stock: 5, displayOrder: 1 },
        ],
      });
      await setupMocks(page, eventData);
      await page.goto(EVENT_PAGE_URL);
      ...
      await expect(page.getByLabel(/Variant B.*Last few! Stock: 5/)).toBeVisible();
    });

    test('2.1.2: should render select dropdown for > 3 variants', async ({ page }) => {
      const eventData = createMockEvent({
        variants: [
      ...
      await expect(page.getByRole('option', { name: /V A.*Stock: 10/ })).toBeVisible();
      await expect(page.getByRole('option', { name: /V B.*Sold out/ })).toBeVisible();
      await expect(page.getByRole('option', { name: /V C.*Last few! Stock: 3/ })).toBeVisible();
      await expect(page.getByRole('option', { name: /V D.*Stock: 30/ })).toBeVisible();
    });
  });
  ...
  test.describe('EventForm - Stock-Aware Quantity Control', () => {
    const eventData = createMockEvent({
      variants: [
        { id: 'v-normal', title: 'Normal Stock', price: 20, stock: 15, displayOrder: 0 }, // More than EVENT_MAX_TICKETS
        { id: 'v-limited', title: 'Limited Stock', price: 20, stock: 3, displayOrder: 1 }, // Less than EVENT_MAX_TICKETS
        { id: 'v-soldout', title: 'Sold Out', price: 20, stock: 0, displayOrder: 2 },
      ],
    });
    test.beforeEach(async ({ page }) => {
      await setupMocks(page, eventData);
    ...
    test('3.1: select variant with ample stock (limited by EVENT_MAX_TICKETS)', async ({ page }) => {
      ...
      await expect(quantityInput).toHaveAttribute('max', String(EVENT_MAX_TICKETS));
    ...
    test('3.2: select variant with limited stock (less than EVENT_MAX_TICKETS)', async ({ page }) => {
    ...
    test('3.3: select sold-out variant', async ({ page }) => {
      ...
      await expect(page.locator('form button').filter({ hasText: /paypal/i }).first()).toBeDisabled();
    });
  });

  test.describe('EventForm - Add-on Descriptions', () => {
    const eventData = createMockEvent({ // Uses default extras from createMockEvent
      eventExtras: [
         { id: 'e1', title: 'Event T-Shirt', price: 25, description: 'Official event merchandise, 100% cotton.', currency: 'GBP' },
         { id: 'e2', title: 'Sticker Pack', price: 5, description: null, currency: 'GBP' }, // No description
      ]
    });
    test.beforeEach(async ({ page }) => {
      await setupMocks(page, eventData);
    ...
    test('4.1: should display description for add-on that has it', async ({ page }) => {
      ...
      const tShirtContainer = tShirtLabel.locator('xpath=ancestor::div[contains(@class, "flex") and contains(@class, "items-start")]');
      
      const description = tShirtContainer.locator('p');
      await expect(description).toBeVisible();
      await expect(description).toHaveText('Official event merchandise, 100% cotton.');
    ...
    test('4.2: should not display description for add-on without it', async ({ page }) => {
      const stickerLabel = page.getByText('Sticker Pack', { exact: true });
      const stickerContainer = stickerLabel.locator('xpath=ancestor::div[contains(@class, "flex") and contains(@class, "items-start")]');
  ...
  test.describe('EventForm - Loading/Error States', () => {
    test('5.1: should display loading indicator during order creation', async ({ page }) => {
      const eventData = createMockEvent({ variants: [{ id: 'v1', title: 'Test', price: 10, stock: 1, displayOrder: 0 }]});
      await setupMocks(page, eventData, {
      ...
      await page.route('**/api/trpc/order.createOrder*', async route => {
        ...
        await route.fulfill({ json: [{ result: { data: { json: { id: 'mock_order_id_123', status: 'CREATED' } } } }] });
      });
      ...
      const payPalButtonContainer = page.locator('form div').filter({ hasText: 'Total' }).locator('xpath=./following-sibling::div[1]'); // Try to locate PayPal button area
      ...
      await expect(page.getByText('Preparing your order...')).toBeVisible({timeout: 500}); // Visible quickly
      await promise; // wait for network call to finish
      await expect(page.getByText('Preparing your order...')).not.toBeVisible();

    });

    test('5.2: should display custom error message on createOrder failure', async ({ page }) => {
      const eventData = createMockEvent({ variants: [{ id: 'v1', title: 'Test', price: 10, stock: 1, displayOrder: 0 }]});
      const errorMessage = "The selected ticket may have recently sold out or changed. Please refresh and try again.";
      await setupMocks(page, eventData, {
        create: { error: "stock issue", details: { /* any other tRPC error details */ } }
      });
      await page.goto(EVENT_PAGE_URL);
      await page.getByLabel(/Test/).click();
      
      // Similar to above, triggering the PayPal flow to get to the error.
      const payPalButtonContainer = page.locator('form div').filter({ hasText: 'Total' }).locator('xpath=./following-sibling::div[1]');
